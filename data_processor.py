...
class DataProcessor:
    ...
    def process_company_data(self, symbol):
        try:
            ...
            # Handle missing sharesOutstanding gracefully by estimating using market cap and share price
            try:
                share_count = self.api_client.get_financial_data("key-metrics", symbol, "sharesOutstanding")
                if not share_count:
                    print(f"Missing 'sharesOutstanding' for {symbol}, attempting fallback...")
                    share_price = self.api_client.get_share_price(symbol)
                    if share_price and market_cap:
                        share_count = [mc / sp for mc, sp in zip(market_cap, share_price)]
                        print(f"Estimated 'sharesOutstanding' for {symbol}: {share_count}")
                    else:
                        raise ValueError(f"'sharesOutstanding' and fallback inputs missing for {symbol}")
            except Exception as e:
                print(f"Error fetching 'sharesOutstanding' for {symbol}: {e}")
                raise
            ...
            return {
                ...
                "SBC / OCF": sbc_ocf[-1],
                "5yr ∆ Share Count": delta_share_count,
